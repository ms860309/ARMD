...
class Copier(object):
    ...
    def __call__(self, *var_args):
        if self.kw is not None:
            ...
            var_kwargs = {key: val for (key, val) in zip(
                self.kw, var_args[-num_var_kw:])}
...
def timeFn(fn):
    ...
    def fnWithTime(*args, **kwargs):
        ...
        args[0].logger.info('{} completed in {:.2f} s'.format(
            fn.__name__, final_time - start_time))
        return result
    return fnWithTime
...
def logStartAndFinish(fn):
    ...
    def fnWrappedWithLog(*args, **kwargs):
        ...
        args[0].logger.info('{} initiated on {}\n'.format(
            fn.__name__, time.asctime()))
        result = fn(*args, **kwargs)
        args[0].logger.info('{} terminated on {}'.format(
            fn.__name__, time.asctime()))
        args[0].logger.info(
            '----------------------------------------------------------------------\n')
        return result
