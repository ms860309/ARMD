...
class Gaussian(Quantum):
    ...
    def __init__(self, input_file=None, logfile=None, chkfile=None):
        super(Gaussian, self).__init__(
            input_file=input_file, logfile=logfile, chkfile=chkfile)
    ...
    def getNumAtoms(self):
        ...
        for line in self.output:
            if read:
                ...
                read = True
        raise QuantumError(
            'Number of atoms could not be found in Gaussian output')
    ...
    def getGradient(self):
        ...
        # Make negative to get gradient
        gradient = - self._formatArray(force_mat_str) / _constants.BOHR2ANG
        return gradient
    ...
    def getGeometry(self):
        ...
        for line_num, line in enumerate(reversed(self.output)):
            ...
            raise QuantumError(
                'Geometry could not be found in Gaussian output')
    ...
    def getIRCpath(self):
        ...
        for line in self.output:
            ...
            raise QuantumError(
                'Gaussian output does not contain IRC calculation')
        ...
        for line_num, line in enumerate(self.output):
            if 'Input orientation' in line or 'Z-Matrix orientation' in line:
                coord_mat = self._formatArray(
                    self.output[line_num + 5:line_num + 5 + natoms])
            elif 'SCF Done' in line:
                energy = float(line.split()[4])
            elif 'Forces (Hartrees/Bohr)' in line:
                force_mat_str = self.output[line_num + 3:line_num + 3 + natoms]
                gradient = - \
                    self._formatArray(force_mat_str) / constants.bohr_to_ang
    ...
    def makeInputFile(self, node, name='gau', jobtype='force', direction='forward', output_dir='',
        ...
        try:
            ...
            print(
                'An error occurred while creating the Gaussian input file', file=sys.stderr)
            raise
        ...
        try:
            ...
            if e.errno == os.errno.ENOENT:
                self.submitProcessAndCheck(
                    'g03', self.input_file, self.logfile)
            else:
                raise
...
class QChem(Quantum):
    ...
    def __init__(self, input_file=None, logfile=None, chkfile=None):
        super(QChem, self).__init__(input_file=input_file,
                                    logfile=logfile, chkfile=chkfile)
    ...
    def getNumAtoms(self):
        ...
        for line in self.output:
            if read:
                ...
                read = True
        raise QuantumError(
            'Number of atoms could not be found in Q-Chem log file')
    ...
    def getGradient(self):
        ...
        for line_num, line in enumerate(reversed(self.output)):
            ...
            raise QuantumError(
                'Gradient could not be found in NWChem log file')

        # Remove every 4th line and 1st column, and extract and flatten gradient components
        grad_mat_str = [row.split()[1:]
                        for (i, row) in enumerate(grad_mat_str) if i % 4]
        ...
        gradient = np.column_stack(
            (xgrad, ygrad, zgrad)) / _constants.BOHR2ANG
        return gradient
    ...
    def getGeometry(self):
        ...
        for line_num, line in enumerate(reversed(self.output)):
            ...
            raise QuantumError(
                'Geometry could not be found in Q-Chem log file')
    ...
    def getIRCpath(self):
        ...
        for line in self.output:
            ...
            raise QuantumError(
                'Q-Chem output does not contain IRC calculation')
        ...
        for line_num, line in enumerate(self.output):
            if 'Standard Nuclear Orientation' in line:
                coord_mat = self._formatArray(
                    self.output[line_num + 3:line_num + 3 + natoms])
    ...
    def makeInputFile(self, node, name='qc', jobtype='force', direction=1, output_dir='',
        ...
        if theory == 'pm6':
            raise QuantumError(
                'PM6 level of theory is not available in Q-Chem')
...
class NWChem(Quantum):
    ...
    def getNumAtoms(self):
        ...
        for line_num, line in enumerate(self.output):
            if 'XYZ format geometry' in line:
                ...
                return natoms
        raise QuantumError(
            'Number of atoms could not be found in NWChem log file')
    ...
    def getGradient(self):
        ...
        gradient = self._formatArray(grad_mat_str) / _constants.BOHR2ANG
        return gradient
    ...
    def getGeometry(self):
        ...
        for line_num, line in enumerate(reversed(self.output)):
            ...
            raise QuantumError(
                'Geometry could not be found in NWChem log file')
    ...
    def getIRCpath(self):
        ...
        for line_num, line in enumerate(self.output):
            if 'Geometry "geometry"' in line:
                ts_mat = self._formatArray(
                    self.output[line_num + 7:line_num + 7 + natoms])
            if any(s in line for s in ('Total SCF energy', 'Total DFT energy')):
                ts_energy = float(line.split()[4])
                break
        else:
            raise QuantumError(
                'TS geometry/energy could not be found in NWChem log file')
        ...
        for line_num, line in enumerate(self.output):
            ...
            raise QuantumError(
                'Forward IRC path could not be found in NWChem log file')
        ...
        for line_num, line in enumerate(reversed(self.output)):
            if 'Geometry "geometry"' in line:
                coord_mat = self._formatArray(
                    self.output[-(line_num - 6):-(line_num - 6 - natoms)])
            if 'Optimization converged' in line:
            ...
            raise QuantumError(
                'Reverse IRC path could not be found in NWChem log file')
    ...
    def executeJob(self, node, name='nwc', output_dir='', nproc=1, **kwargs):
        ...
        try:
            ...
            if e.errno == os.errno.ENOENT:
                self.submitProcessAndCheck(
                    'mpiexec', '-n', nproc, 'nwchem', self.input_file, self.logfile)
            else:
                raise
