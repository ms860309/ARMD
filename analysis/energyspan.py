...
for i, line in enumerate(rough_content):
    if line.__contains__('reactant_smiles:') or line.__contains__('product_smiles:'):
        reactant_smiles = line.split(':')[1].rstrip().split('.')
        ...
        node_identical.append(reactant_part_smiles)
node_identical = remove_same_element(node_identical)
# print(node_identical)
...
with open(refine_dir, "w") as out:
    for line in rough_content:
        ...
        if line.__contains__('reactant_smiles:') or line.__contains__('product_smiles:'):
            ...
            reactant_part_smiles = set(reactant_part_smiles)
            if len(reactant_part_smiles) > 1:
                reactant_part_smiles = '.'.join(reactant_part_smiles)
            else:
                reactant_part_smiles = ''.join(reactant_part_smiles)
            out.write(line.replace(tmp, label_dict[reactant_part_smiles]))
...
# with open(output_energy_span_dir, "w") as f:
